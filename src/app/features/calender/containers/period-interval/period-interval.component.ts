...
export class PeriodIntervalComponent implements OnInit {
  ...
  updateInterval(interval: {duration: number; text: string}) {
    this.store.dispatch(setPeriodInterval({periodInterval: interval.duration}))
  }
  ngOnInit(): void {
    ...
    this.teachingState.subscribe(
      teachingState => {
        const _intervalObjIdx = this.intervals.findIndex(interval => 
          interval.duration+'' === (teachingState.classesAndGroups[0] as ClassGroupModel).periods[0].intervaBtwPeriods)
        this.periodIntervalForm = new FormGroup({
          interval: new FormControl(this.intervals[_intervalObjIdx] ? this.intervals[_intervalObjIdx] : null, {validators: Validators.required})
        })
      }
    )
  }
