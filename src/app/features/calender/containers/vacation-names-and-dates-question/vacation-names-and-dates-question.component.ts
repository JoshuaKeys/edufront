...
import { addVacation, setVacationName, setVacationStartDate, setVacationEndDate } from '../../ngrx/actions/calendar.actions';
...
export class VacationNamesAndDatesQuestionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.store.select(selectCalendar)
    // .pipe(first())
    .subscribe(
      calendarState => {
        ...
        const formGroups = calendarState.vacations.map(vacation => {
          return new FormGroup({
            vacationName: new FormControl(vacation.vacationName, { updateOn: 'blur' }),
        ...
        this.vacationForm.controls.vacationsAndDates['controls'].forEach(
          (formGroup: FormGroup, idx) => {
            ...
            formGroup.controls.vacationName.valueChanges.subscribe(vacationName => {
