...
import { setTermName, setTermStartDate, setTermEndDate } from '../../ngrx/actions/calendar.actions';
  ...
  ngOnInit(): void {
    ...
    this.calendarData.pipe(
      ...
      (calendarData) => {
        ...
        this.termsAndDatesForm.controls.termsAndDates['controls'].forEach((formGroup: FormGroup, idx)=> {
          formGroup.controls.termName.valueChanges.subscribe(
            termName => this.store.dispatch(setTermName({idx, termName}))
          )
          formGroup.controls.startDate.valueChanges.subscribe(
            startDate => this.store.dispatch(setTermStartDate({idx, startDate}))
          )
          formGroup.controls.endDate.valueChanges.subscribe(
            endDate => this.store.dispatch(setTermEndDate({idx, endDate}))
          )
        })
