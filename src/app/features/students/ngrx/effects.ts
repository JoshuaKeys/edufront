...
import {
  toggleAddModal,
  toggleEditModal
} from './actions/students-modal.actions';
...
export class StudentsEffects {
  uploadStudentsViaExcel$ = createEffect(() =>
    this.actions$.pipe(
      ofType(uploadStudentsViaExcel),
      withLatestFrom(
        this.store.select(selectAllStudents),
        this.store.select(selectAllClasses)
      ),
      mergeMap(([action, existingStudents, allClasses]) =>
        this.staffStudentSharedService.uploadProfileApi(action.file).pipe(
          catchError(error => of(uploadStudentsViaExcelError({ error }))),
          mergeMap(students => {
            let createArr = [];
            let updateArr = [];

            students.forEach(student => {
              let _student = JSON.parse(JSON.stringify(student));
              let _classObj = allClasses.find(
                _class => _class.id == _student.classId
              );
              _student.className = Boolean(_classObj) ? _classObj.name : '';
              let currentStudentWithSameEmail = existingStudents.find(
                existingStudent => existingStudent.profileDto.id === _student.id
              );
              if (currentStudentWithSameEmail !== undefined) {
                updateArr.push(_student);
              } else {
                createArr.push(_student);
              }
            });

            return [
              updateStudents({ students: updateArr }),
              insertStudents({ students: createArr })
            ];
          })
        )
      )
  ...
  initStudentsXClasses$ = createEffect(() =>
    this.actions$.pipe(
        ...
        forkJoin(
          ...
          mergeMap(([classes, students]) => {
            const mappedStudents = students.map(student => {
              ...
              // console.log('student - ', student.className);
              return student;
            });
            // console.log('initClassesAndStudentsResponse - ', mappedStudents);
            return [
              initClassesAndStudentsResponse({
                studentsXclasses: mappedStudents
              }),
              fetchedClassesSuccess({ classes }),
              fetchedStudentsSuccess({
                students: students.map(student => ({ profileDto: student }))
              })
            ];
          })
        )
  ...
  fetchStudentData$ = createEffect(() =>
    this.actions$.pipe(
      ofType(fetchStudentByIdRequest),
      mergeMap(action =>
        this.studentsService
          .getStudentById(action.student)
          .pipe(
            mergeMap(student => [
              fetchStudentByIdResponse({ student }),
              toggleEditModal()
            ])
          )
      )
  ...
  refreshStudentsXClasses$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([action, students, classes]) => {
        const mappedStudents = <any>students.map(student => {
          ...
          for (let i = 0; i < classes.length; i++) {
            if (classes[i].id === student.profileDto.classId) {
              (student.profileDto as StudentsXClassesModel).className =
                classes[i].name;
              break;
            }
          }
        ...
        return initClassesAndStudentsResponse({
          studentsXclasses: mappedStudents
        });
      })
    )
  ...
  editStudentRequest$ = createEffect(() =>
    this.actions$.pipe(
      ofType(editStudentRequest),
      mergeMap(action => {
        const studentReqData = this.processStudentReqData(action.student);
        studentReqData.profileDto.countryId =
          studentReqData.profileDto['country'].id;
        studentReqData.profileDto.phone =
          studentReqData.guardianDetailsDto.phone['phoneNum'] || '';
        studentReqData.guardianDetailsDto.phone =
          studentReqData.guardianDetailsDto.phone['phoneNum'] || '';
        ...
        if (studentReqData.profileDto.profileImage) {
          if (studentReqData.profileDto.profileImage['imageUrl'].length > 0) {
            studentReqData.profileDto.profileImage =
              studentReqData.profileDto.profileImage['imageUrl'];
            return this.studentsService
              .editStudent(studentReqData)
              .pipe(
                mergeMap(student => [
          ...
          const fileConvertedFromBase64 = dataURLtoFile(
            studentReqData.profileDto.profileImage['base64'],
            'profilePic.png'
          );
          return this.studentsService.uploadLogo(fileConvertedFromBase64).pipe(
            mergeMap(res => {
              console.log(res);
              const editStudentObj = this.composeCreateStudentData(
                studentReqData,
                res.file
              );
              return this.studentsService
  ...
  deleteStudentRequest$ = createEffect(() =>
    this.actions$.pipe(
      ofType(deleteStudentRequest),
      mergeMap(action =>
        this.studentsService
          .deleteStudent(action.student)
          .pipe(map(res => deleteStudentSuccess({ student: action.student })))
      )
    )
  );
