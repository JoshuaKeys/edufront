...
export const classesReducer = createReducer(classesInitialState,
  on(getAllClassesSuccess, (state, action) => {
    return classesAdapter.addAll(
      action.classes, state
    )
  }),
  on(toggleSelectedState, (state, action) => {
    const allClasses = selectAll(state);
    const allClassesCopy: ClassesModel[] = JSON.parse(JSON.stringify(allClasses));
    const classIndex = allClassesCopy.findIndex(classItem => classItem.class.name === action.classGrade)
    allClassesCopy[classIndex].class.selected = !allClassesCopy[classIndex].class.selected;
    const clearedClasses: Update<ClassesModel>[] = removePreviouslySelectedState(allClassesCopy, classIndex);
  ...
  on(toggleStudentsDraggedState, (state, action) => {
    const allClassesCopy: ClassesModel[] = JSON.parse(JSON.stringify(selectAll(state)));
    ...
    allClassesCopy[classToEdit].students[studentToEdit].dragged =
      !allClassesCopy[classToEdit].students[studentToEdit].dragged;
    return classesAdapter.updateOne({
  ...
  on(assignStudentsRandomly, (state, action) => {
    ...
    const mappedClassesCopy = allClassesCopy[classToEdit].students.map(
      item => {
        item.dragged = true;
        return item;
      }
