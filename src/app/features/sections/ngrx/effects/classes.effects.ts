  ...
  getAllClassesRequest$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(([action, allClasses]) => {
      ...
      if (allClasses.length > 0) {
        classStaffObj = allClasses.map(classItem => ({ class: classItem.class, students: classItem.students }))
        ...
        return this.studentsService.uploadLogo(studentReqData.profileDto.profileImage['acceptedFile']).pipe(
          mergeMap(res => {
            ...
            return this.studentsService.createStudent(createStudentReqObj).pipe(
              mergeMap(student => [createStudentSuccess({ student }), toggleModal({ modal: 'addModal' })])
            )
          })
        )
      }
