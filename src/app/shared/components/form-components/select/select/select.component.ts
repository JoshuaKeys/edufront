import { Component, OnInit,  Output,  Input,EventEmitter , ChangeDetectionStrategy, HostListener,forwardRef, ChangeDetectorRef, ViewChild, ContentChildren , QueryList } from '@angular/core';
...
export class SelectComponent  implements OnInit,  ControlValueAccessor  {
  ...
  cbBlur(){ //change to rxjs operator later
    setTimeout(()=>{
    ...
    switch(keyCode){
      ...
      case this.DOWN_ARROW_KEY_CODE:
          if(this.activeOptionIndex === null){
            ...
            newIndex = (this.activeOptionIndex + 1 +this.optionEls.length)%this.optionEls.length;
          }
          // this.selectService.activeOptionIndex.next(newIndex);
        // this.selectService.setActiveOption(this.optionEls.toArray()[this.activeOptionIndex].OptionValue);
        this.selectService.activeOptionComponent.next( this.optionEls.toArray()[newIndex] );
